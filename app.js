...
app.get('/api/items', async (req, res) => {
  try {
    const [rows] = await pool.promise().query('SELECT id, nama_product, harga_product, gambar_product FROM product ORDER BY id DESC');
    ...
    const items = rows.map(item => ({
        ...
        price: item.harga_product,
        imageUrl: item.gambar_product
}));
...
app.post('/api/items', upload.single('image'), async (req, res) => {
  try {
    ...
    await pool.promise().query(
      'INSERT INTO product (nama_product, harga_product, gambar_product) VALUES (?, ?, ?)',
